  ...
  return (data ?? []).map((col) => ({
    ...
    items: (col.collection_items ?? []).map((item) => {
      ...
      return {
        ...
        status: usedInShow ? 'used' : (item.status ?? 'available'),
...
export async function getChecklistTemplate(seriesId) {
  ...
  const { data, error } = await supabase
    ...
    .select('id, task, category, condition, default_owner, weeks_out, enabled, sort_order, comm_template_id, tags')
    .eq('series_id', seriesId)
...
export async function getDutyTemplates(seriesId) {
  ...
  const { data, error } = await supabase
    .from('duty_templates')
    .select('id, default_assigned_to, duty, time_note, sort_order, comm_template_id')
    .eq('series_id', seriesId)
    .order('sort_order', { ascending: true })
...
// ─── Performers available to add to a series ─────────────────────────────────

export async function getAvailablePerformers(seriesId) {
  const supabase = await createClient()

  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return []

  const { data: inSeries } = await supabase
    .from('performer_series')
    .select('performer_id')
    .eq('series_id', seriesId)

  const existingIds = (inSeries ?? []).map((r) => r.performer_id)
  ...
  if (error) {
    console.error('getAvailablePerformers error:', error.message)
    return []
  }
  return data ?? []
}
...
export async function getSeriesPerformers(seriesId, { page = 0, pageSize = 50 } = {}) {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      performer_id,
      performers (
        id, name, act_type, instagram, email, contact_method, book_again, audience_favourite,
        show_performers (
          performer_id,
