 ...
 *
 * Assumed schema:
 *   series          (id, owner_id, name, is_one_off, frequency, show_type, venue_name)
 *   shows           (id, series_id, title, date, status)
 *   checklist_items (id, show_id, done, due_date)
 *   show_performers (id, show_id, performer_id, status)   status: 'confirmed'|'invited'|'cancelled'
  ...
  const series = (data ?? []).map((s) => {
    ...
    const upcomingShows = allShows
      ...
      .map((show) => {
        ...
        const confirmedPerformers = performers.filter(
