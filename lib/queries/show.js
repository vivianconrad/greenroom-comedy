 ...
 *   show_performers (id, show_id, performer_id, role, status, slot_order,
...
export async function getShowDuties(showId) {
  ...
  const { data, error } = await supabase
    .from('show_duties')
    .select('id, assigned_to, duty, time_note, done, sort_order')
    .eq('show_id', showId)
...
export async function getShowDetail(showId) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      ...
      show_performers (
        id, performer_id, role, status, slot_order, set_length, call_time,
      ...
      show_duties (
        id, assigned_to, duty, time_note, done, sort_order
      )
    `
    )
    .eq('id', showId)
