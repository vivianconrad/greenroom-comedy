...
export async function getShowDuties(showId) {
  ...
  const { data, error } = await supabase
    .from('show_duties')
    .select('id, assigned_to, duty, time_note, completed, sort_order, comm_template_id')
    .eq('show_id', showId)
...
export async function getShowDetail(showId) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      ...
      call_time, doors_time, show_time,
      tickets_sold, capacity, ticket_price, ticket_platform, ticket_url, promo_code,
      venue_cost,
      notes_attendance, notes_rating, notes_energy,
      notes_worked, notes_didnt_work, notes_next_time,
      series (
        ...
        comm_templates ( id, name, body, tags, sort_order )
      ),
      checklist_items (
        id, task, done, due_date, category, stage,
        owner, weeks_out, enabled, sort_order, template_id, comm_template_id, tags
      ),
      show_performers (
        ...
        name_pronunciation, plugs, special_needs,
        performers (
      ...
      show_duties (
        id, assigned_to, duty, time_note, completed, sort_order, comm_template_id
      )
    `
    )
    .eq('id', showId)
  ...
  const performers = (data.show_performers ?? [])
    ...
    .map((sp) => ({
      ...
      name_pronunciation: sp.name_pronunciation ?? null,
      plugs: sp.plugs ?? null,
      special_needs: sp.special_needs ?? null,
...
/**
 * Returns performers the user owns who are NOT already booked on this show.
 */
export async function getAvailablePerformersForShow(showId) {
  const supabase = await createClient()
  ...
  const existingIds = (existing ?? []).map((r) => r.performer_id).filter(Boolean)

  let query = supabase
    .from('performers')
    .select('id, name, stage_name, act_type, instagram, clip_url')
    .eq('owner_id', user.id)
  ...
  if (error) {
    console.error('getAvailablePerformersForShow error:', error.message)
    return []
  }
  return data ?? []
}
