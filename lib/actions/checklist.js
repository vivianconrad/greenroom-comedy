...
/**
 * Bulk-sets enabled on checklist_items using at most 2 queries (one per
...
async function bulkUpdateChecklistItems(supabase, items) {
  ...
  const trueIds = items.filter((i) => i.enabled).map((i) => i.id)
  const falseIds = items.filter((i) => !i.enabled).map((i) => i.id)

  const [trueResult, falseResult] = await Promise.all([
    trueIds.length
      ? supabase.from('checklist_items').update({ enabled: true }).in('id', trueIds)
      : { error: null },
    falseIds.length
      ? supabase.from('checklist_items').update({ enabled: false }).in('id', falseIds)
      : { error: null },
  ])
...
/**
 * Bulk-sets enabled on checklist_templates for items that carry a template_id.
...
async function bulkUpdateTemplates(supabase, items) {
  ...
  // Deduplicate: build template_id → enabled map (last value wins, but in
  // practice one template maps to exactly one enabled state per edit batch).
  const templateMap = new Map()
  for (const item of withTemplate) {
    templateMap.set(item.template_id, item.enabled)
  }
  ...
  const [trueResult, falseResult] = await Promise.all([
    trueIds.length
      ? supabase.from('checklist_templates').update({ enabled: true }).in('id', trueIds)
      : { error: null },
    falseIds.length
      ? supabase.from('checklist_templates').update({ enabled: false }).in('id', falseIds)
      : { error: null },
  ])
...
/**
 * Saves enabled changes to the current show's checklist only.
...
/**
 * Saves enabled changes to the current show's checklist AND to the series
...
/**
 * Saves enabled changes to the current show's checklist, to the series
 * templates, and pushes the changes to all other upcoming shows in the series.
 *
 * The "push" step calls the `push_checklist_enabled_to_upcoming_shows`
 * Postgres function (supabase/migrations/20260322000000_create_push_checklist_active_to_upcoming_shows_rpc.sql)
...
export async function saveChecklistToTemplateAndPush(seriesId, showId, items) {
  ...
  if (itemsWithTemplates.length > 0) {
    const { error } = await supabase.rpc('push_checklist_enabled_to_upcoming_shows', {
...
export async function addChecklistItem(showId, taskData) {
  ...
  try {
    ...
    const task = taskData.task ?? taskData.name
    ...
    if (existing > 0) return { error: 'A checklist item with that task already exists for this show.' }
    ...
    const { error } = await supabase.from('checklist_items').insert({
      ...
      owner: taskData.owner ?? taskData.default_owner ?? null,
...
/**
 * Sets (or clears) the owner on a single checklist item.
 */
export async function updateChecklistItemOwner(itemId, owner) {
  const supabase = await createClient()
  try {
    const user = await getAuthenticatedUser(supabase)
...
/**
 * Toggles enabled on a single checklist item (used outside of batch edit mode).
 */
