...
export async function createSeries(formData) {
  ...
  try {
    ...
    // Guards against duplicate insertion (e.g. double form submission) by
    // checking whether any rows already exist for this series before inserting.
    async function insertFromJson(key, table, mapFn) {
      ...
      try {
        ...
        console.error(`createSeries: failed to parse ${key} JSON`)
        return
      }
      ...
      if (count > 0) return
      const { error: insertError } = await supabase.from(table).insert(items.map(mapFn))
      if (insertError) console.error(`createSeries: failed to insert ${table}:`, insertError.message)
    }
    ...
    if (isOneOff) {
      ...
      if (rawDates) {
        let dates = []
        try {
          dates = JSON.parse(rawDates).filter(Boolean)
        } catch {
          // malformed dates JSON — skip show creation, series is already saved
        }
        if (dates.length > 0) {
          const now = new Date()
          ...
          const { error: showsError } = await supabase.from('shows').insert(
            dates.map((date) => ({
              series_id: series.id,
              date,
          ...
          if (showsError) return { error: showsError.message }
        }
      }

      revalidatePath('/dashboard')
      revalidatePath(`/dashboard/series/${series.id}`)
...
export async function removeContactFromSeries(seriesId, contactId) {
  const supabase = await createClient()
  try {
    const user = await getAuthenticatedUser(supabase)
    await verifySeriesOwnership(supabase, seriesId, user.id)
    ...
    const updated = (row.contacts ?? []).filter((c) => c.id !== contactId)
...
export async function permanentlyDeleteSeries(seriesId) {
  const supabase = await createClient()
  try {
    const user = await getAuthenticatedUser(supabase)
    await verifySeriesOwnership(supabase, seriesId, user.id)

    // Hard-delete all shows in the series first (or rely on DB cascade)
    await supabase.from('shows').delete().eq('series_id', seriesId)
