...
export async function fetchPerformerForEdit(performerId) {
  const supabase = await createClient()
  try {
    const user = await getAuthenticatedUser(supabase)
    const { data, error } = await supabase
      .from('performers')
      .select('id, name, stage_name, pronouns, act_type, instagram, email, contact_method, how_we_met, clip_url, notes, tags, book_again, audience_favourite')
      .eq('id', performerId)
      .eq('owner_id', user.id)
      .single()
    if (error || !data) return { error: 'Not found or not authorized.' }
...
export async function convertContactToPerformer(contact, seriesId) {
  const supabase = await createClient()
  try {
    const user = await getAuthenticatedUser(supabase)
    ...
    const { data: performer, error: createError } = await supabase
      .from('performers')
      .insert({
        owner_id: user.id,
        ...
        contact_method: contact.phone ? 'Phone' : null,
      })
      .select('id')
      .single()
    ...
    // Link performer to the series (ignore duplicate)
    const { error: linkError } = await supabase
      .from('performer_series')
      .insert({ performer_id: performer.id, series_id: seriesId })

    if (linkError && linkError.code !== '23505') return { error: linkError.message }

    // Remove the contact from the series JSONB
    ...
    const updated = (row.contacts ?? []).filter((c) => c.id !== contact.id)
