...
export async function updateDuty(dutyId, data) {
  ...
  try {
    ...
    const patch = {}
    if (data.assigned_to !== undefined) patch.assigned_to = data.assigned_to?.trim() || null
...
export async function toggleDutyCompleted(dutyId) {
  ...
  try {
    ...
    const { data: duty } = await supabase
      .from('show_duties')
      .select('completed, show_id, shows ( series_id )')
      .eq('id', dutyId)
      .single()
...
export async function updateDutyTemplateLink(templateId, commTemplateId) {
  ...
  try {
    ...
    // Propagate to existing show_duties generated from this template
    await supabase
      .from('show_duties')
      .update({ comm_template_id: commTemplateId ?? null })
