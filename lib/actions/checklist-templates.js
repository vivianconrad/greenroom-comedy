...
export async function createChecklistTemplate(seriesId, { task, name, category, condition, default_owner, weeks_out, tags }) {
...
export async function updateChecklistTemplateOwner(taskId, owner) {
  const supabase = await createClient()
  try {
    const user = await getAuthenticatedUser(supabase)
    ...
    const { error } = await supabase
      .from('checklist_templates')
      .update({ default_owner: owner?.trim() || null })
      .eq('id', taskId)
