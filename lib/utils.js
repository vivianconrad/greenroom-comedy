 ...
 *
 * Supported variables: [date] [callTime] [doors] [showTime] [venue] [theme] [runningOrder] [ticketUrl] [promoCode]
 */
export function fillTemplate(body, showData) {
  ...
  return body
    ...
    .replace(/\[theme\]/g, showData.theme ?? '[theme]')
    .replace(/\[runningOrder\]/g, ro)
    .replace(/\[ticketUrl\]/g, showData.ticket_url ?? '[ticketUrl]')
    .replace(/\[promoCode\]/g, showData.promo_code ?? '[promoCode]')
}
