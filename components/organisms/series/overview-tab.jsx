...
import { ContactCard, LoginCard } from '@/components/organisms/series/info-tab'
...
export function SeriesOverviewTab({ series, seriesId }) {
  ...
  function triggerAutoSave() {
    ...
    timerRef.current = setTimeout(() => {
      startTransition(async () => {
        const result = await updateSeriesInfo(seriesId, dataRef.current)
  ...
  function addContact() {
    setLocalData((prev) => ({
      ...prev,
      contacts: [...prev.contacts, { id: crypto.randomUUID(), name: '', role: '', phone: '', email: '', notes: '' }],
    }))
    triggerAutoSave()
  }
  ...
  function addLogin() {
    setLocalData((prev) => ({
      ...prev,
      logins: [...prev.logins, { id: crypto.randomUUID(), service: '', username: '', password: '', url: '', notes: '' }],
    }))
    triggerAutoSave()
  }
  ...
  return (
    <div className="pt-6 space-y-8">
      ...
      <section>
        ...
        >
          <h3 className="font-display text-lg text-deep">Logins</h3>
          <span className="text-xs text-soft font-body transition-colors group-hover:text-mid">
            {loginsOpen ? '▾ hide' : '▸ show'}
          </span>
          {localData.logins.length > 0 && (
            <span className="text-xs text-soft font-body">({localData.logins.length})</span>
          )}
        </button>
        <p className="text-xs text-soft font-body mb-4">
        ...
        {loginsOpen && (
          <>
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <Button variant="secondary" size="sm" onClick={addLogin} className="ml-4 shrink-0">+ Add login</Button>
            </div>
          </>
        )}
      </section>
