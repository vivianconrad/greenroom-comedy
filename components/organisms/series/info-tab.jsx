...
export function CopyField({ label, value, onChange, placeholder, type = 'text' }) {
  return (
...
export function ContactCard({ entry, onChange, onRemove }) {
...
export function LoginCard({ entry, onChange, onRemove }) {
...
// ─── Performers table (read-only) ─────────────────────────────────────────────

function PerformersTable({ performers }) {
  return (
    <div className="rounded-card border border-peach bg-white overflow-hidden">
      <table className="w-full">
        <thead className="border-b border-peach bg-cream/50">
          <tr>
            <th className="px-4 py-2.5 text-left text-xs font-medium text-soft uppercase tracking-wide">Name</th>
            <th className="px-4 py-2.5 text-left text-xs font-medium text-soft uppercase tracking-wide w-28">Act type</th>
            <th className="px-4 py-2.5 text-left text-xs font-medium text-soft uppercase tracking-wide w-36">Contact</th>
            <th className="px-4 py-2.5 text-left text-xs font-medium text-soft uppercase tracking-wide w-20">Shows</th>
            <th className="px-4 py-2.5 w-8" />
          </tr>
        </thead>
        <tbody>
          {performers.map((p) => {
            const contact = p.instagram
              ? `@${p.instagram}`
              : p.email ?? null
            return (
              <tr key={p.id} className="border-b border-peach last:border-0 hover:bg-cream/50 transition-colors group">
                <td className="px-4 py-3">
                  <span className="text-sm font-body text-deep font-medium">{p.name}</span>
                  {p.stage_name && (
                    <span className="block text-xs font-body text-soft/70">"{p.stage_name}"</span>
                  )}
                </td>
                <td className="px-4 py-3 text-xs font-body text-soft w-28">
                  {p.act_type ?? '—'}
                </td>
                <td className="px-4 py-3 text-xs font-body text-soft w-36 truncate">
                  {contact ?? '—'}
                </td>
                <td className="px-4 py-3 text-xs font-body text-soft/60 tabular-nums w-20">
                  {p.showCount}
                </td>
                <td className="px-4 py-3 w-8">
                  <Link
                    href="/dashboard/performers"
                    className="text-xs text-soft/30 group-hover:text-coral transition-colors"
                  >
                    →
                  </Link>
                </td>
...
export function InfoTab({ series, seriesId, performers = [] }) {
    ...
    <div className="pt-6 max-w-2xl space-y-10">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-soft font-body mt-0.5">
              Venue manager, sound engineer, photographer — people you work with every show cycle.
            </p>
          </div>
      ...
      {performers.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-display text-lg text-deep">Performers</h3>
              <p className="text-xs text-soft font-body mt-0.5">
                From your performer database — manage them in the Performers tab.
              </p>
            </div>
            <Link
              href="/dashboard/performers"
