...
function ChecklistStep({ presets, selected, onToggle, onSelectAll, onSelectNone, custom, onAddCustom, onRemoveCustom }) {
  const [showForm, setShowForm] = useState(false)
  ...
  const allSelected = presets.length > 0 && presets.every((p) => selected.has(p.task))
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        ...
        <button
          ...
          className="self-start text-xs text-coral hover:text-coral/70 font-body font-medium transition-colors"
        >
...
export function CreateSeriesModal({ open, onClose }) {
  ...
  function doSubmit({ skipCollections = false } = {}) {
      ...
      <form ref={formRef} onSubmit={handleFormSubmit} className="flex flex-col gap-6" noValidate>
        ...
        {step === 2 && (
          <ChecklistStep
            ...
            onSelectAll={() => setSelectedChecklist(new Set(checklistPresets.map((p) => p.task)))}
            onSelectNone={() => setSelectedChecklist(new Set())}
          ...
          <div className="flex items-center gap-2">
            {step >= 2 && step < TOTAL_STEPS && (
              <Button type="button" variant="ghost" size="md" disabled={isPending}
                onClick={() => {
                  if (step === 2) skipStep(() => { setSelectedChecklist(new Set()); setCustomChecklist([]) })
                  else if (step === 3) skipStep(() => { setSelectedComms(new Set()); setCustomComms([]) })
            ...
            {step === TOTAL_STEPS && (
              <Button type="button" variant="ghost" size="md" disabled={isPending}
                onClick={() => doSubmit({ skipCollections: true })}>
                Skip
              </Button>
            )}
            {step < TOTAL_STEPS ? (
